...
public interface GpRepository extends JpaRepository<GP, Long> {
    ...
    @Query(value ="SELECT R.GRid AS GRid, R.name AS GRname, T.GTid AS GTid, T.name AS GTname, P.GPid AS GPid, P.name AS GPname, P.img AS img, P.state AS state, P.text AS text " +
            "FROM GP P " +
