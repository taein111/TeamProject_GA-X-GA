...
public interface GpRepository extends JpaRepository<GP, Long> {
    @Query(value ="SELECT R.GRid AS GRid, R.name AS GRname, T.GTid AS GTid, T.name AS GTname, P.GPid AS GPid, P.name AS GPname, P.img AS img " +
            "FROM GP P " +
...
//    @Query(value = "SELECT * FROM GP WHERE ")
//    @Query(value = "SELECT R.name, T.name, P.name, P.img FROM GP P JOIN GR R ON R.GRid = P.GRid JOIN GT T ON T.GTid = P.GTid", nativeQuery = true)
...
//
    @Query(value = "SELECT img FROM GP", nativeQuery = true)
