...
public interface GpRepository extends JpaRepository<GP, Long> {
    ...
    @Query(value ="SELECT P.GPid AS GPid, P.name AS GPname, P.img AS img, P.state AS state, P.text AS text, P.jjimcount AS jjimCount, R.GRid AS GRid, R.name AS GRname, T.GTid AS GTid, T.name AS GTname " +
            "FROM GP P " +
